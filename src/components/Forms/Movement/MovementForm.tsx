...
type MovementFormProps = {
  ...
  onPurchaseRequisitionChange?: (
    purchaseRequisition: PurchaseRequisition
  ) => void;
  onMovementTypeChange?: (movementTypeChange: MovementType) => void;
};

const MovementForm = forwardRef<MovementFormHandle, MovementFormProps>(
  (_props, ref) => {
    const [form] = Form.useForm();
    const [isDraft, setIsDraft] = useState<boolean>(false);
    const [purchasesRequisitions, setPurchasesRequisitions] = useState<
      GetPurchaseRequisitionsResponse[]
    >([]);
    const [workOrders, setWorkOrders] = useState<GetWorkOrdersResponse[]>([]);
    const [movementConcepts, setMovementConcepts] = useState<
      GetMovementConceptsResponseData[]
    >([]);
    const [movementTypes, setMovementTypes] = useState<
      GetMovementTypesResponseData[]
    >([]);
    const [warehouses, setWarehouses] = useState<GetWarehousesResponse[]>([]);
    const [warehousesToShow, setWarehousesToShow] = useState<
      GetWarehousesResponse[]
    >([]);
    const [disableOriginWarehouse, setDisableOriginWarehouse] =
      useState<boolean>(true);
    const [date, setDate] = useState<moment.Moment>(moment());

    const onMovementConceptSelected = (value: string) => {
      const movementConcept = movementConcepts.find(
        (concept) => concept.id === value
      );
      _props.onMovementConpetChange
        ? _props.onMovementConpetChange(movementConcept)
        : null;
      const movementType = movementTypes.find(
        (type) => type.id === movementConcept?.movementTypeId
      );
      ...
      onMovementTypeChange(movementType?.id);
      form.setFieldsValue({
    ...
    const showWorkOrderSelector = (movementConceptId: string): boolean => {
      const movementConcept = movementConcepts.find(
        (concept) => concept.id === movementConceptId
      );
      return (
        movementConcept?.name?.toLowerCase() ==
        "Adquisición de mercancía".toLowerCase()
      );
    };

    const showRequisitionRelatedOcSelector = (
      ...
      return (
        movementConcept?.name?.toLowerCase() ==
        "Salida a producción".toLowerCase()
      );
    };
    ...
    const onPurchaseRequisitionSelected = (value: string) => {
      const purchaseRequisition = purchasesRequisitions.find(
        (purchaseRequisition) => purchaseRequisition.id === value
      );
      _props.onPurchaseRequisitionChange
        ? _props.onPurchaseRequisitionChange(purchaseRequisition)
        : null;
    };

    const onMovementTypeChange = (value: string) => {
      const movementType = movementTypes.find((type) => type.id === value);
      _props.onMovementTypeChange
        ? _props.onMovementTypeChange(movementType)
        : null;
    };
    ...
    useEffect(() => {
      ...
      const getPurchasesRequisitions = async () => {
        try {
          const purchasesRequisitions =
            await PurchaseRequisitionAPI.findPurchaseRequisitions();
    ...
    return (
      <Form
        ...
        ) : showRequisitionRelatedOcSelector(
            form.getFieldValue("movementConceptId")
          ) ? (
          <Row gutter={24}>
            <Col span={24}>
              <Form.Item
